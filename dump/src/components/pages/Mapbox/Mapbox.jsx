...
const MAPBOX_TOKEN =
  "pk.eyJ1IjoiZ2Fzcy1kZXYiLCJhIjoiY2tmNWlrMWs3MG5sMzJ5bm8wNmZzbzVxaCJ9.hAUlN6_W3j6zbC--NqYUkg";

require("./_mapbox.scss");
//process.env.
  ...
  useEffect(() => {
    const geocode = async () => {
      let response = await Axios.get(
        "http://localhost:8000/api//post_reports/post"
      );
      let reports = response.data.posts.map(async (post) => {
        ...
        let result = await Axios.get(
          `https://api-adresse.data.gouv.fr/search/?q=${numberStreet}+${post.street}+${postalCode}+${post.city}`
        );
